...
import (
	...
	"github.com/luxfi/evmgpu/precompile/contracts/txallowlist"
...
// overrides reports whether an EVM running under rules reaches the Lux
// precompile at addr: what the EVM asks their Payload when a CALL targets addr.
func overrides(rules params.Rules, addr common.Address) bool {
	o, ok := rules.Payload.(vm.PrecompileOverrider)
	if !ok {
		return false
	}
	_, ok = o.PrecompileOverride(addr)
	return ok
}
...
func (r ruleSet) check() (string, bool) {
	...
	if overrides(rules, txallowlist.ContractAddress) != r.upgrade {
		return "the tx allow list precompile", false
	}
...
// The precompiles a CALL reaches are those of the time the EVM's own Rules
// were evaluated at: the overrider in their Payload decides from that time,
// whichever Rules were evaluated after it, in either order.
func TestPrecompileOverrideIsTheRulesOwn(t *testing.T) {
	config := upgradedAt(100)
	before := config.Rules(common.Big1, params.IsMergeTODO, 99)
	at := config.Rules(common.Big1, params.IsMergeTODO, 100)

	require.False(t, overrides(before, txallowlist.ContractAddress), "an EVM before the upgrade reached the tx allow list")
	require.True(t, overrides(at, txallowlist.ContractAddress))

	_ = config.Rules(common.Big1, params.IsMergeTODO, 99)
	require.True(t, overrides(at, txallowlist.ContractAddress), "an EVM at the upgrade lost the tx allow list")
	require.False(t, overrides(before, txallowlist.ContractAddress))
}
